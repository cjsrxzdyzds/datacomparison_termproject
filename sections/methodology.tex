...
We implement a custom \textbf{Simple Recurrent Neural Network (RNN)} from scratch in MATLAB to avoid the overhead of the Deep Learning Toolbox for symbol-by-symbol updates.
\begin{itemize}
    \item \textbf{Architecture}: Input layer (one-hot), Hidden layer (tanh activation), Output layer (Softmax).

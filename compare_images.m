function compare_images() % Define images img1_path = 'data/Desert.gif';
img2_path = 'data/Sky and birds.gif';

if
  ~exist(img1_path, 'file') || ~exist(img2_path, 'file')
                                   error('Images not found in data/');
end

    % Read images[I1, map1] = imread(img1_path);
if
  ~isempty(map1) I1 = ind2gray(I1, map1);
end

    [I2, map2] = imread(img2_path);
if
  ~isempty(map2) I2 = ind2gray(I2, map2);
end
...
close(h);
    ...
    function H = compute_entropy(poly) counts = imhist(poly);
p = counts / sum(counts);
p = p(p > 0);
H = -sum(p.*log2(p));
end
